...
### 고쳤다 (2026-08-26, 이 표를 뽑은 뒤)

원인은 한 줄 더 아래였다. `run_provider(collect_usage=...)`는 `None`을 "주변
기본값을 써라"로, `False`를 "쓰지 마라"로 읽는다. 그 주변 기본값을 세우는 게
`providers.run.recording(collect_usage=True)`다. 평가 하네스가 원장 없이
토큰을 요청하는 방법이 바로 그것이다. `dobby/runtime/workers.py`가 원장이
없을 때 명시적 `False`를 넘겨서 recorder의 `True`를 이겼다.

이제 원장이 없으면 `None`을 넘긴다. 실측으로 확인했다 — recorder 안에서
codex 한 번 호출해 `codex exec --json`이 16,491 입력 토큰을 돌려줬다.
회귀 테스트 5개를 붙였고, 결함을 일시적으로 되돌리면 그중 2개가 실패한다.

**위 표의 dobby 토큰 값은 그대로 둔다.** 수정 후 값을 얻으려면 열다섯 번을
다시 사야 한다. 사지 않았다. 그러니 표의 1,122,506은 여전히 바닥값이다.
