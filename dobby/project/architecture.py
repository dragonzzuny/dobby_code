...
def build_prompt(request: ArchitectureRequest, *, item, manifest) -> str:
    ...
    return "\n".join([
        ...
        *(f"  {c}" for c in allowed),
        ...
        "",
        "## The other two vocabularies, which are also closed",
        "This module's own rule is that telling you the boundary up front turns",
        "a rejection into a plan. It did that for commands and not for these,",
        "and a real call was spent proposing a side-effect class that does not",
        "exist.",
        "",
        f"`side_effect_class` must be exactly one of {list(SAFE_SIDE_EFFECTS)}."
        " Use LOCAL_WRITE when the work edits files in this repository. Naming",
        "anything else — however reasonable it sounds — halts for a human.",
        "",
        "Each entry in `execution_steps` must carry a `role` that is one of",
        "['scout', 'implement', 'critic'], an `objective`, and — for the one",
        "`implement` step — a `write_set` naming the files it will change.",
        "There may be at most ONE implement step: nothing here isolates two",
        "writers in one tree. `verify` and `report` are added by the harness and",
        "may not be proposed.",
        "",
        "## Required output",
